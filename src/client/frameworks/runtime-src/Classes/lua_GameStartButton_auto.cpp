...
int lua_GameStartButton_GameStartButton_create(lua_State* tolua_S)
{
    int argc = 0;
    bool ok  = true;
    ...
    if (argc == 0)
    {
        if(!ok)
        {
            tolua_error(tolua_S,"invalid arguments in function 'lua_GameStartButton_GameStartButton_create'", nullptr);
            return 0;
        }
        bubble_second::GameStartButton* ret = bubble_second::GameStartButton::create();
        object_to_luaval<bubble_second::GameStartButton>(tolua_S, "bs.GameStartButton",(bubble_second::GameStartButton*)ret);
        return 1;
    }
    luaL_error(tolua_S, "%s has wrong number of arguments: %d, was expecting %d\n ", "bs.GameStartButton:create",argc, 0);
    return 0;
#if COCOS2D_DEBUG >= 1
    tolua_lerror:
    tolua_error(tolua_S,"#ferror in function 'lua_GameStartButton_GameStartButton_create'.",&tolua_err);
#endif
    return 0;
}
...
int lua_register_GameStartButton_GameStartButton(lua_State* tolua_S)
{
    ...
    tolua_beginmodule(tolua_S,"GameStartButton");
        ...
        tolua_function(tolua_S,"create", lua_GameStartButton_GameStartButton_create);
