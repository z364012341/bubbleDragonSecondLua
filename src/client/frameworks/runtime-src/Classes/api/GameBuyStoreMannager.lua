
--------------------------------
-- @module GameBuyStoreMannager
-- @parent_module bs

--------------------------------
-- 
-- @function [parent=#GameBuyStoreMannager] buyProps 
-- @param self
-- @param #int target
-- @return GameBuyStoreMannager#GameBuyStoreMannager self (return value: bs.GameBuyStoreMannager)
        
--------------------------------
-- 
-- @function [parent=#GameBuyStoreMannager] isBuyPuzzleNewbiwGift 
-- @param self
-- @return bool#bool ret (return value: bool)
        
--------------------------------
-- 
-- @function [parent=#GameBuyStoreMannager] getRandomLotteryAwardWihtoutDecals 
-- @param self
-- @return map_table#map_table ret (return value: map_table)
        
--------------------------------
-- 
-- @function [parent=#GameBuyStoreMannager] getRandomLotteryAwardIncludeCharactorDecals 
-- @param self
-- @return map_table#map_table ret (return value: map_table)
        
--------------------------------
-- 
-- @function [parent=#GameBuyStoreMannager] getInstance 
-- @param self
-- @return GameBuyStoreMannager#GameBuyStoreMannager ret (return value: bs.GameBuyStoreMannager)
        
return nil
